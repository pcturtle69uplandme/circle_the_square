...
|---|-------|------|------|--------|-------|
| 1 | F01 | — | Jan, Christina, Office | ⚠️ **unresolved: wide shot vs. dialogue conflict** | Two MiniMax-H3 attempts both had real problems: v2 (turbo) had unusable audio (see below), v3 (standard+EasyCache, `F01_v3.mp4`, seed 201002) had correct audio and beat but **the whole wide office shot reads blurry/low-res**, not just faces — 864x480 doesn't carry enough detail across a full wide environment. Google Flow was considered next but is blocked by Flow's likeness/content filters. The new `wan22-pipeline/` (Wan 2.2 TI2V-5B) fixes the wide-shot sharpness problem — but **F01 has a real dialogue line** (Christina: "Morning Jan. Survive the weekend?") and Wan produces no audio at all, so it can't just be swapped in like a silent establishing shot can. Not resolved — needs a decision: (a) accept MiniMax's blur for this one shot since it at least has correct audio, (b) split F01 into a silent Wan wide shot + an immediate cut to a MiniMax close-up that actually carries the line (standard filmmaking coverage, but changes the shot count), or (c) generate the line as separate audio (TTS) and mux it onto the Wan visual (adds a real audio/lip-sync pipeline not yet built). Do not regenerate F01 again until this is decided. |
