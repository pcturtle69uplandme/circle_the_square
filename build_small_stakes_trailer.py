...
small_stakes_audio = os.path.join(BASE_DIR, "audio-refs", "Small Stakes.mp3")
output_master = os.path.join(clips_dir, "SMALL_STAKES_TRAILER_MASTER_60S.mp4")

if not os.path.exists(small_stakes_audio):
    sys.exit(f"Error: Could not find audio file at {small_stakes_audio}")

print(f"=== Merging Trailer with Background Music Track: {small_stakes_audio} ===")
...
title_video_end = os.path.join(clips_dir, "temp_title_end_small_stakes_5s.mp4")
cmd_title_end = [
    FFMPEG, "-y",
    "-loop", "1",
    "-i", title_img,
...
# Mix Small Stakes background audio [bg_small_stakes] + [dialogue_mix] with balanced weights
audio_filter = (
    ...
    f"[10:a]volume='if(lt(t,21.0), 1.8, if(lt(t,22.5), 1.8 - (t-21.0)*(1.8-0.85)/1.5, 0.85))':eval=frame,atrim=0:{total_duration:.2f},aresample=48000[bg_small_stakes];"
    f"[bg_small_stakes][dialogue_mix]amix=inputs=2:weights=1.1 2.8:duration=longest:dropout_transition=0,afade=t=out:st={fade_out_start:.2f}:d=1.5,aresample=48000[aout]"
)
...
cmd_master.extend(["-i", small_stakes_audio])
...
print(f"\n--- Assembling Trailer with 'Small Stakes.mp3' (Fade Out at {fade_out_start:.2f}s) ---")
...
if res.returncode == 0 and os.path.exists(output_master):
    print(f"\n[SUCCESS] Master Trailer created successfully!")
