...
async function main() {
  ...
  console.log('Clicking the Circle the Square project link directly in DOM...');
  ...
  await page.waitForURL('**/project/f2f0d2c9**', { timeout: 20000 });
  await page.waitForTimeout(4000);
  console.log('Successfully inside project URL:', page.url());

  // 1. Locate the visible composer
...
Wide still frame inside executive office. Jan Peach (52-year-old overweight cartoon executive with thinning comb-over, round face, gold watch on wrist, orange PRISM lanyard around his neck) is in a flustered cartoon comedy state, having discarded his pale blue dress shirt onto his desk chair. He is wiping his flushed, sweaty brow, breathing heavily and stressed. Background is strictly his executive office with black-and-white geometric triangle acoustic wall, dark wooden bookcase on the right, and walnut desk.`;
  ...
  console.log('Filling policy-safe prompt...');
  ...
  // 3. Click Create button (arrow_forward Create at x=1276, y=951)
  console.log('Clicking Create button...');
  await page.mouse.click(1276, 951);
  console.log('Create clicked! Polling for newly generated image...');

  // 4. Poll for new image
  ...
  if (newUrl) {
    console.log('Downloading high-res image binary from Flow context...');
    ...
    console.log(`[OK] Successfully saved F19 master frame to ${outPath} (${fs.statSync(outPath).size} bytes)`);
  } else {
    await page.screenshot({ path: 'character-refs/stencils/f19_nav_timeout.png' });
    console.log('Saved f19_nav_timeout.png');
  }
...
main().catch(err => {
  console.error('Error in navigate_and_run_f19:', err.message);
  process.exit(1);
});
